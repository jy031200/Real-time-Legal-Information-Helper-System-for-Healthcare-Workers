...
class MyWindow(QMainWindow, MainCode.Ui_zx):
    # 값이 0일 때는 법률팁 영상 크롤링, 1일 때는 하지 않음
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.lawyer_sample()

        self.btn1.clicked.connect(self.tip_exec)
        self.btn1.clicked.connect(self.content_combo_search)
        self.pushButton_16.clicked.connect(self.lawyer_search)
        self.pushButton_4.clicked.connect(self.video_search)
        self.pushButton_0.clicked.connect(self.tip_exec)
        self.pushButton_0.clicked.connect(self.content_search)
        self.setWindowIcon(QtGui.QIcon('background.png'))

    def lawyer_sample(self): #변호사 추천
        #추천 변호사 리스트
    ...
    def video_search(self): #법률팁 검색창 검색
        driver = webdriver.Chrome('chromedriver.exe')
        ...
        channel = driver.find_elements_by_class_name("style-scope ytd-channel-name") #채널명 불러오기
        for a in channel:
            ytb_channel.append(a.text) #리스트에 넣기
        ...
        for i in range(0, 9):
            vi_tb = "self.webview_" + str(ytb_num) + ".load(QUrl(\"" + str(ytb_link[i]) + "\"))"  # 영상 넣는 명령어
            exec(vi_tb)  # 명령어 실행
            ytb_num += 1  # 레이블 값 변경
    ...
    def content_search(self): #판례 검색창 검색
        ...
        if (Site == "로시컴"):  # 사이트가 로시컴일 경우
            ...
            image = wait.until(
                EC.visibility_of_element_located((By.CLASS_NAME, 'case-card__title')))  # 클래스 이름이 나올 때까지 대기
    ...
    def tip_exec(self): #로톡 법률TIP 불러오기
        global tip
        global tip_url  # 링크 리스트
        global video_title  # 제목 리스트
        global video_lawyer
